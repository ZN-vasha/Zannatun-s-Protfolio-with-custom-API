$(document).ready(function () {
  ...
  const contact_service = "service_ebv8ne6";
  const template_contact = "template_4rkboz7";
  $("#contact-form").submit(function (event) {
    emailjs.init("8EwrriKuTojo_4CXd");
...
document.addEventListener("visibilitychange", function () {
  if (document.visibilityState === "visible") {
    document.title = "Portfolio | Zannatun";
...
async function fetchData1() {
  let response;
  response = await fetch("/abilities.json");
...
function showAbilities(abilities) {
  ...
  abilities.forEach((ability) => {
    abilityHTML += `
        <div class="bar">
              <div class="info">
                <img src=${ability.icon} alt="skill" />
